...
def run_viewport(browser, url: str, name: str) -> tuple[list[Control], int, list[str]]:
    """Measure one viewport. Returns (controls, analytics_blocked, analytics_completed)."""
...
def _report(name: str, controls: list[Control], blocked: int, completed: list[str]) -> int:
    print(f"\n=== {name} ({VIEWPORTS[name]['viewport']['width']}x"
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            worst = EXIT_PASS
            payload: dict = {"url": args.url, "floor_px": FLOOR_PX, "viewports": {}}
            for name in names:
                try:
                    controls, blocked, completed = run_viewport(browser, args.url, name)
                except RuntimeError as e:
                    print(f"\n=== {name} ===\nBLIND: {e}", file=sys.stderr)
                    worst = max(worst, EXIT_BLIND)
                    payload["viewports"][name] = {"outcome": "blind", "error": str(e)}
                    continue
                rc = _report(name, controls, blocked, completed)
                # A funnel leak is a failure in its own right, independent of sizes.
                if completed:
                    rc = max(rc, EXIT_FAIL)
                worst = max(worst, rc)
                payload["viewports"][name] = {
